<?php
    ...
    class forumAdminView extends forum {
        ...
        function init()
        {
            ...
            if($module_info && $module_info->module != 'forum')
		...
		$comment_count = $comment_part_config->comment_count;
        	...
        	if(count($newest_documents->data) == 1) 
        		$newest_docs[] = $newest_documents->data;
        	else
        		$newest_docs = $newest_documents->data; 
		...
		if(!Context::get('module_srl'))
			return $this->dispForumAdminContent();
		if(!in_array($this->module_info->module, array('admin', 'forum','blog','guestbook')))
			return $this->alertMessage('msg_invalid_request');
